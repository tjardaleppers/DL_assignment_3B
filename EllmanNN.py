...
class EllmanNN(nn.Module):
    def __init__(self, vocab_size, emb_size, hidden_size):
        super(EllmanNN, self).__init__()
        self.embedding = nn.Embedding(vocab_size, emb_size)
        self.ellman = ElmanLayer(emb_size, hidden_size, hidden_size)
        self.ReLU = nn.ReLU()
        self.linear = nn.Linear(hidden_size, 2)

    def forward(self, x, hidden_prev):
        x = self.embedding(x)
        # Save the hidden layer for next it
        ellman_out, hidden = self.ellman(x, hidden_prev)
        output_hidden = self.ReLU(ellman_out)
...
class ElmanLayer(nn.Module):
    ...
    def forward(self, x, hidden_prev=None):
        b, t, e = x.size()
        # If no hidden layer values yet, set to zero
        if hidden_prev is None:
            hidden_prev = torch.zeros(b, e, dtype=torch.float)
        outs = []        
        for i in range(t):
            inp = torch.cat([x[:, i, :], hidden_prev], dim=1)
            hidden = self.lin1(inp)
            # Detach hidden layer to prevent memory issues
...
def trainModel(params):
    ...
    vocab_size = len(i2w)
    model = EllmanNN(vocab_size, emb_size, hidden_size)
    ...
    # Save model
    torch.save(model.state_dict(), "EllmanNN.pth")                      
